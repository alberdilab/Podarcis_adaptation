...
ggarrange(beta_richness_nmds_post1, beta_neutral_nmds_post1, beta_phylogenetic_nmds_post1, ncol=3, nrow=1, common.legend = TRUE, legend="right")
```
...
ancom_rand_output = ancombc2(data = physeq_FMT1, 
                  assay_name = "counts", 
                  tax_level = NULL,
...
```{r ancom_rand_FMT1, comment="", echo=FALSE, message=FALSE, warning=FALSE, eval=FALSE}
...
```{r ancombc_rand_plot_phy_FMT1, comment="", echo=FALSE, message=FALSE, warning=FALSE, fig.height=10, fig.width=8, fig.fullwidth=TRUE, eval=FALSE}
ancombc_rand_table_mag%>%
...
```{r sign_features_FMT1_warm, comment="", message=FALSE, warning=FALSE, eval=FALSE}
ancombc_rand_table_mag%>%
...
```{r sign_features_FMT1_cold, comment="", message=FALSE, warning=FALSE, eval=FALSE}
ancombc_rand_table_mag%>%
  filter(lfc_PopulationHot_dry<0)  %>% 
  count(phylum, name = "sample_count") %>%
  arrange(desc(sample_count))  
...
```{r}
physeq_FMT1 <- subset_samples(physeq_all, type %in% c("Control", "Treatment") & time_point == "FMT1")
physeq_FMT1 <- prune_taxa(taxa_sums(physeq_FMT1)>0, physeq_FMT1)
```
```{r ancom_rand_pond_FMT1_cold, comment="", message=FALSE, warning=FALSE, eval=FALSE}
ancom_rand_output_cold = ancombc2(data = physeq_FMT1, 
                  assay_name = "counts", 
                  tax_level = NULL,
                  fix_formula = "type", 
                  p_adj_method = "holm", 
...
```{r ancom_rand_FMT1_cold, comment="", echo=FALSE, message=FALSE, warning=FALSE, eval=FALSE}
taxonomy <- data.frame(physeq_FMT1@tax_table) %>%
  rownames_to_column(., "taxon") %>%
  mutate_at(vars(order, phylum, family, genus, species), ~ str_replace(., "[dpcofgs]__", ""))
...
```{r ancombc_rand_plot_phy_FMT1_cold, comment="", echo=FALSE, message=FALSE, warning=FALSE, fig.height=10, fig.width=8, fig.fullwidth=TRUE, eval=FALSE}
ancombc_rand_table_mag%>%
      mutate(genome=factor(taxon,levels=ancombc_rand_table_mag$taxon)) %>%
...
***Phyla of the significant MAGs in intervention group***
```{r sign_features_FMT1_int, comment="", message=FALSE, warning=FALSE, eval=FALSE}
ancombc_rand_table_mag%>%
...
```{r sign_features_FMT1_cold_con, comment="", message=FALSE, warning=FALSE, eval=FALSE}
ancombc_rand_table_mag%>%
  filter(lfc_typeTreatment<0)  %>% 
  count(phylum, name = "sample_count") %>%
  arrange(desc(sample_count))  
...
ggarrange(beta_richness_nmds_FMT2, beta_neutral_nmds_FMT2, beta_phylogenetic_nmds_FMT2, ncol=3, nrow=1, common.legend = TRUE, legend="right")
```
...
```{r ancom_rand_FMT2, comment="", echo=FALSE, message=FALSE, warning=FALSE, eval=FALSE}
...
```{r ancombc_rand_plot_phy_FMT2, comment="", echo=FALSE, message=FALSE, warning=FALSE, fig.height=10, fig.width=8, fig.fullwidth=TRUE, eval=FALSE}
ancombc_rand_table_mag%>%
      mutate(genome=factor(taxon,levels=ancombc_rand_table_mag$taxon)) %>%
ggplot(., aes(x=lfc_PopulationHot_dry, y=forcats::fct_reorder(genome,lfc_PopulationHot_dry), fill=phylum)) + #forcats::fct_rev()
  geom_col() + 
...
***Phyla of the significant MAGs in warm control***
```{r sign_features_FMT2_warm, comment="", message=FALSE, warning=FALSE, eval=FALSE}
ancombc_rand_table_mag%>%
  filter(lfc_PopulationHot_dry>0)  %>% 
  count(phylum, name = "sample_count") %>%
  arrange(desc(sample_count))  
...
***Phyla of the significant MAGs in intervention group***
```{r sign_features_FMT2_cold, comment="", message=FALSE, warning=FALSE, eval=FALSE}
ancombc_rand_table_mag%>%
  filter(lfc_PopulationHot_dry<0)  %>% 
  count(phylum, name = "sample_count") %>%
  arrange(desc(sample_count))  
...
```{r ancom_rand_FMT2_cold, comment="", echo=FALSE, message=FALSE, warning=FALSE, eval=FALSE}
taxonomy <- data.frame(physeq_FMT2@tax_table) %>%
  rownames_to_column(., "taxon") %>%
  mutate_at(vars(order, phylum, family, genus, species), ~ str_replace(., "[dpcofgs]__", ""))
...
```{r ancombc_rand_plot_phy_FMT2_cold, comment="", echo=FALSE, message=FALSE, warning=FALSE, fig.height=10, fig.width=8, fig.fullwidth=TRUE, eval=FALSE}
ancombc_rand_table_mag%>%
      mutate(genome=factor(taxon,levels=ancombc_rand_table_mag$taxon)) %>%
ggplot(., aes(x=lfc_typeTreatment, y=forcats::fct_reorder(genome,lfc_typeTreatment), fill=phylum)) + #forcats::fct_rev()
  geom_col() + 
...
***Phyla of the significant MAGs in intervention group***
```{r sign_features_FMT2_int, comment="", message=FALSE, warning=FALSE, eval=FALSE}
ancombc_rand_table_mag%>%
  filter(lfc_typeTreatment>0)  %>% 
  count(phylum, name = "sample_count") %>%
  arrange(desc(sample_count))  
...
***Phyla of the significant MAGs in cold control***
```{r sign_features_FMT2_cold_con, comment="", message=FALSE, warning=FALSE, eval=FALSE}
ancombc_rand_table_mag%>%
  filter(lfc_typeTreatment<0)  %>% 
  count(phylum, name = "sample_count") %>%
  arrange(desc(sample_count))  
