...
label_map <- c(
  ...
  "functional" = "Functional Diversity"
)
bxp1 <- alpha_div %>%
...
#  facet_nested(.~metric+type, labeller = labeller(metric = label_map, type = label_map))+
  ...
  stat_compare_means(size=3, show.legend = FALSE) +
  theme_minimal() +
...
***Richness***
```{r alpha_div_data_post3_comparison, comment="", message=FALSE, warning=FALSE, eval=FALSE}
...
***Richness***
```{r alpha_div_data_post4_comparison, comment="", message=FALSE, warning=FALSE, eval=FALSE}
Modelq0GLMMNB <- glm.nb(richness ~ type*time_point, data = alpha_div_meta,trace=TRUE)
summary(Modelq0GLMMNB)
emmeans(Modelq0GLMMNB, pairwise ~ type)
```
...
***Richness***
```{r alpha_div_data_comparison, comment="", message=FALSE, warning=FALSE, eval=FALSE}
Modelq0GLMMNB <- glm.nb(richness ~ type*time_point, data = alpha_div_meta,trace=TRUE)
summary(Modelq0GLMMNB)
emmeans(Modelq0GLMMNB, pairwise ~ type)
```
