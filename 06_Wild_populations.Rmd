...
ggarrange(beta_q0n_nmds_wild, beta_q1n_nmds_wild, beta_q1p_nmds_wild, ncol=3, nrow=1, common.legend = TRUE, legend="right")
```
...
structural_zeros <- genome_counts_filt %>% 
   ...
   mutate(present = case_when(
      all_zeros_cold & !all_zeros_warm ~ "cold",
      !all_zeros_cold & all_zeros_warm ~ "warm",
      !all_zeros_cold & !all_zeros_warm ~ "None",
      TRUE ~ NA_character_
    )) %>%
   mutate(average = ifelse(present == "cold", average_cold, average_warm)) %>%
...
physeq_sample_wild <- subset_samples(physeq_genome_filtered, time_point %in% c("Wild"))
physeq_sample_wild<- prune_taxa(taxa_sums(physeq_sample_wild)>0, physeq_sample_wild)
```

### Phylum
```{r ancom_rand_pond_phylum, comment="", message=FALSE, warning=FALSE, eval=FALSE}
ancom_rand_output_wild_phylum = ancombc2(data = physeq_sample_wild, 
                  assay_name = "counts", 
                  tax_level = "phylum",
...
### MAG level
```{r ancom_rand_pond_mag_wild_data, comment="", message=FALSE, warning=FALSE, eval=FALSE}
...
ancom_rand_output_mag_wild = ancombc2(data = physeq_wild_mag, 
                  assay_name = "counts", 
                  tax_level = NULL,
                  fix_formula = "Population", 
                  p_adj_method = "holm", 
