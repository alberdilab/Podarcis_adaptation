...
```{r beta_neutral_nmds, echo=FALSE, warning=FALSE, comments="", message=FALSE, results=TRUE}
...
```{r beta_neutral_nmds_post1, echo=FALSE, warning=FALSE, comments="", message=FALSE, results="hide"}
...
```{r beta_neutral_nmds_post2, echo=FALSE, warning=FALSE, comments="", message=FALSE, results="hide"}
...
```{r structure_plot_1, echo=TRUE, results=TRUE}
p3 <-merge_gift_wild %>%
...
### GIFT test visualisation (needs to be fix)
...
GIFTs_functions_community %>%
  ...
  #left_join(sample_metadata, by = join_by(sample == Tube_code)) %>%
  #separate(sample, into = c("time_point", "type"), sep = "\\.") %>%
  #pivot_longer(-c(time_point, type), names_to = "trait", values_to = "value") %>%
  mutate(trait = case_when(
