...
5 samples are removed from the analysis due to their low sequencing depth.

```{r filterging, echo=TRUE, warning=FALSE}
#Counts_raw
columns_to_exclude <- c("AD16","AD23", "AD25", "AD69", "AD91") # Columns to exclude
...
data_stats %>%
  ...
  pivot_longer(-sample) %>%
  #pivot_longer(cols = c(low_quality, mapped_lizard, mapped_mags, unmapped_reads), 
...
p3<-beta_q1f_nmds_wild %>%
			...
			mutate(y_cen = mean(NMDS2, na.rm = TRUE)) %>%
			ungroup() %>%
			ggplot(., aes(x=NMDS1,y=NMDS2, color=species, label=sample)) +
				scale_color_manual(values=c("#e5bd5b","#6b7398")) +
				geom_point(size=2) +
				geom_segment(aes(x=x_cen, y=y_cen, xend=NMDS1, yend=NMDS2), alpha=0.2) +
...
pairwise<-pairwise.adonis(beta_div_richness_post1$S,post1$type, perm=999)
pairwise
```
...
pairwise<-pairwise.adonis(beta_div_neutral_post1$S,post1$type, perm=999)
pairwise
```
...
pairwise<-pairwise.adonis(beta_div_phylo_post1$S,post1$type, perm=999)
pairwise
```
...
pairwise<-pairwise.adonis(beta_div_func_post1$S,post1$type, perm=999)
pairwise
```
...
pairwise<-pairwise.adonis(beta_div_richness_post2$S,post2$type, perm=999)
pairwise
```
...
pairwise<-pairwise.adonis(beta_div_neutral_post2$S,post2$type, perm=999)
pairwise
```
...
pairwise<-pairwise.adonis(beta_div_phylo_post2$S,post2$type, perm=999)
pairwise
```
...
pairwise<-pairwise.adonis(beta_div_func_post2$S,post2$type, perm=999)
pairwise
```
