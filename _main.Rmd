...
```{r beta_div_neutral_accli_1, echo=TRUE, results = FALSE, message=FALSE, warning=FALSE, comments=FALSE}
...
```{r beta_div_neutral_accli1_1, echo=TRUE, results=FALSE,message=FALSE, warning=FALSE, comments=FALSE}
...
adonis2(formula=beta_div_richness_accli1$S ~ time_point+species+individual, data=accli1[labels(beta_div_neutral_accli1$S),], permutations=999) %>%
		as.matrix() %>%
		kable()
```
...
adonis2(formula=beta_div_neutral_accli1$S ~ time_point+species+individual, data=accli1[labels(beta_div_neutral_accli1$S),], permutations=999) %>%
		as.matrix() %>%
		kable()
```
...
adonis2(formula=beta_div_phylo_accli1$S ~ time_point+species+individual, data=accli1[labels(beta_div_phylo_accli1$S),], permutations=999) %>%
		as.matrix() %>%
		kable()
```
...
adonis2(formula=beta_div_func_accli1$S ~ time_point+species+individual, data=accli1[labels(beta_div_func_accli1$S),], permutations=999) %>%
		as.matrix() %>%
		kable()
```
...
```{r diss_data_1, echo=FALSE, results=FALSE}
transplants_metadata<-sample_metadata%>%
  mutate(Tube_code=str_remove_all(Tube_code, "_a"))
transplants_metadata$newID <- paste(transplants_metadata$Tube_code, "_", transplants_metadata$individual)
...
```{r diss_data, echo=FALSE, results=FALSE}
...
```{r diss_plot, echo=FALSE, results=FALSE}
transplant4_beta_div_neutral_met$type<-as.factor(transplant4_beta_div_neutral_met$type)

Acclimation_Transplant<-transplant4_beta_div_neutral_met %>% 
...
Transplant_Treatment<-transplant4_beta_div_neutral_met %>% 
  filter(comparison == "6_Post-FMT2 - 4_Transplant2")
...
Treatment_Acclimation<-transplant4_beta_div_neutral_met %>% 
  filter(comparison == "0_Wild - 6_Post-FMT2")
...
```{r diss_data1_1, echo=FALSE, results=FALSE}
...
```{r diss_plot1, echo=FALSE, results=FALSE}
# Create the plot with the specified facet order
transplant5_beta_div_neutral_met$type<-as.factor(transplant5_beta_div_neutral_met$type)

Acclimation_Transplant<-transplant5_beta_div_neutral_met %>% 
  filter(comparison == "4_Transplant2 - 1_Acclimation")
...
Transplant_Treatment<-transplant5_beta_div_neutral_met %>% 
  filter(comparison == "6_Post-FMT2 - 4_Transplant2")
bxp2 <- Transplant_Treatment %>%
  ggplot(aes(x = type, y = Distance, color = type, fill = type)) +
  geom_boxplot() +
...
Treatment_Acclimation<-transplant5_beta_div_neutral_met %>% 
...
```{r diss_plot2_!, , fig.height=7, eho=FALSE, results=TRUE}
ggarrange( p1, p2, p3, ncol=3, nrow=1, common.legend = TRUE, legend="right")
```
