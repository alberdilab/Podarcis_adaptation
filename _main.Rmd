...
```{r wild_sign_features_warm, comment="", message=FALSE, warning=FALSE, eval=FALSE}
ancombc_rand_table_mag%>%
...
```{r wild_sign_features_cold, comment="", message=FALSE, warning=FALSE, eval=FALSE}
ancombc_rand_table_mag%>%
  filter(lfc_PopulationHot_dry<0)  %>% 
  count(phylum, name = "sample_count") %>%
  arrange(desc(sample_count))  
...
alpha_div %>%
  ...
  ylab("Alpha diversity")
```
***p<0.05 when comparin neutral beta diversity between Cold-control and Warm-control
