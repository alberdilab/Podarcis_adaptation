...
Four samples are removed from the analysis due to their low sequencing depth.

```{r filterging, echo=TRUE, warning=FALSE}
#Counts_raw
columns_to_exclude <- c("AD23", "AD25", "AD69", "AD91") # Columns to exclude
...
### Prokaryotic fraction (missing, not done by the server)
...
data_stats <- raw_reads %>%
  ...
  #left_join(singlem) %>% #try to create this file
...
```{r beta_neutral_nmds, echo=TRUE, warning=FALSE, comments="", message=FALSE, results=TRUE}
...
adonis2(formula=beta_div_richness_post2$S ~ species+type, data=post2[labels(beta_div_richness_post2$S),], permutations=999) %>%
		as.matrix() %>%
		kable()
```
...
adonis2(formula=beta_div_neutral_post2$S ~ species+type, data=post2[labels(beta_div_neutral_post2$S),], permutations=999) %>%
		as.matrix() %>%
		kable()
```
...
adonis2(formula=beta_div_phylo_post2$S ~ species+type, data=post2[labels(beta_div_phylo_post2$S),], permutations=999) %>%
		as.matrix() %>%
		kable()
```
...
adonis2(formula=beta_div_func_post2$S ~ species+type, data=post2[labels(beta_div_func_post2$S),], permutations=999) %>%
		as.matrix() %>%
		kable()
```
...
```{r gitfs_functional_wild, echo=TRUE,results=FALSE}
GIFTs_functions_community %>%
  rowMeans() %>%
  as_tibble(., rownames = "sample") %>%
  left_join(sample_metadata, by = join_by(sample == Tube_code)) %>%
...
```{r gift_test_function_plot_wild, message=FALSE, warning=FALSE, fig.height=12, fig.width=10, fig.fullwidth=TRUE, eval=FALSE}
GIFTs_functions_community %>%
  as.data.frame() %>%
...
```{r gitfs_domain_wild, echo=TRUE,results=FALSE}
...
```{r gitfs_elements_wild, echo=TRUE,results=FALSE}
...
```{r gitfs_functional_tm6, echo=TRUE,results=FALSE}
GIFTs_functions_community %>%
  rowMeans() %>%
  as_tibble(., rownames = "sample") %>%
  left_join(sample_metadata, by = join_by(sample == Tube_code)) %>%
...
```{r gift_test_function_plot, message=FALSE, warning=FALSE, fig.height=12, fig.width=10, fig.fullwidth=TRUE, eval=FALSE}
GIFTs_functions_community %>%
  as.data.frame() %>%
  rownames_to_column("sample") %>%
  left_join(sample_metadata, by = join_by(sample == Tube_code)) %>%
...
```{r gitfs_domain_tm6, echo=TRUE,results=FALSE}
GIFTs_domains_community %>%
  rowMeans() %>%
  as_tibble(., rownames = "sample") %>%
  left_join(sample_metadata, by = join_by(sample == Tube_code)) %>%
...
```{r gitfs_elements_tm6, echo=TRUE,results=FALSE}
GIFTs_elements_community %>%
  rowMeans() %>%
  as_tibble(., rownames = "sample") %>%
  left_join(sample_metadata, by = join_by(sample == Tube_code)) %>%
