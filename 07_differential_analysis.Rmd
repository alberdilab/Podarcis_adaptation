...
element_gift_names_CI_accli_post1 <- element_gift_CI_accli_post1_filter%>%
  select(-time_point)%>%
  t() %>%
  row_to_names(row_number = 1) %>%
  as.data.frame() %>%
  ...
  mutate_if(is.character, as.numeric)  %>%
  rownames_to_column(., "Tube_code")%>% 
  left_join(., sample_metadata_tm9[c(1,10)], by = join_by(Tube_code == Tube_code))


colNames <- names(element_gift_names_CI_accli_post1)[2:13] #check the column that has the last code
for(i in colNames){
